...
# Stats 506, Fall 2020
#
# This script estimates the disparity in internet access between urban and 
...
disp_internet_ur_repl = 
  prop_internet_ur_repl[, 
                        .(delta_internet_r = diff(prop_internet_r)), 
                        .(DIVISION, repl)]
...
# make plots/tables easier to interpret by ordering by disparity: -------------
divisions = c(
  "New England",
  "Middle Atlantic",
  "East North Central",
...
ci_fmt = '%4.1f (%4.1f, %4.1f)'
tab1 = prop_internet_ur[,
 .(division, 
   urban, 
   ci = sprintf(ci_fmt, 100 * prop_internet, 100 * lwr, 100 * upr) 
 ) ] %>%
...
tab1 = 
  disp_internet_ur[, 
   .(division, 
     disparity = sprintf(ci_fmt, 100 * delta_internet, 100 * lwr, 100 * upr) 
   ) ] %>%
...
ci_format = '%4.2f (%4.2f, %4.2f)'

ptab = prop_internet_ur[,
 .(`Census Division` = division,
   urban = str_to_title(urban),
   prop = sprintf(ci_format, prop_internet, lwr, upr)
   )] %>%
  dcast( `Census Division` ~ urban, value.var = 'prop')

dtab = disp_internet_ur[, 
 .(`Census Division` = division, 
   Disparity = sprintf(ci_format, delta_internet, lwr, upr))]

tab = merge(ptab, dtab, by = 'Census Division')

## result for printing
res_fmt = '%s [Urban = %s; Rural = %s]'
ms_disp = tab[`Census Division` == "Mountain South", 
              sprintf(res_fmt, Disparity, Urban, Rural)]
